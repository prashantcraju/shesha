...
`Shesha` has been used as the primary analysis framework in three lines of published research, each corresponding to a distinct module. All analyses in [@raju2026geometric], [@raju2026canary], and [@raju2026crispr] were conducted using `Shesha`, with reproducible artifacts available as seven Colab tutorials (most are executable in under 5 minutes; the vision tutorial is longer because it loads several pretrained models):
