...
from shesha.bio import perturbation_coherence, perturbation_effect_size  # noqa: E402
...
def get_perturbation_groups(adata):
    ...
    if "perturbation" in adata.obs.columns:
        ...
        if candidates:
            ...
            raise ValueError(
                f"Could not find perturbation column. Available: {list(adata.obs.columns)}"
            )
...
def test_on_real_data():
    ...
    for pert in perturbations[:n_test]:
        ...
        print(
            f"{str(pert)[:30]:30s}  n={n_cells:4d}  coherence={coherence:.3f}  effect={effect:.2f}"
        )
