...
def test_class_separation_ratio():
    ...
    np.random.seed(320)
    X = np.vstack([np.random.randn(100, 10), np.random.randn(100, 10) + 5])
...
def test_lda_stability():
    ...
    np.random.seed(320)
    X = np.vstack([np.random.randn(100, 10), np.random.randn(100, 10) + 3])
    y = np.array([0] * 100 + [1] * 100)
...
def test_perturbation_coherence_knn():
    ...
    X_ctrl = np.vstack([np.random.randn(250, 50), np.random.randn(250, 50) + 2])
...
def test_comparison_standard_vs_whitened():
    ...
    # Whitened
    white_stab = shesha.bio.perturbation_coherence_whitened(
        X_ctrl, X_pert, max_samples=100, seed=320
    )
